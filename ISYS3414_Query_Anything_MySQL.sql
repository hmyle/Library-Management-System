...
CREATE VIEW staff_detail AS
SELECT GROUP_CONCAT(CONCAT(staff.first_name, ' ', staff.last_name) ORDER BY staff.last_name SEPARATOR ', ') AS full_name, staff.phone_num, position.name AS position_name
FROM staff
...
CREATE VIEW accountant_detail AS
SELECT CONCAT(s.first_name, ' ', s.last_name) AS accountant_name, a.staff_id AS accountant_id, v.status AS cpa_validation_status
FROM accountant a
JOIN cpa_validation v ON a.cpa_status = v.id
JOIN staff s ON a.staff_id = s.id;

-- Janitor Detail View
CREATE VIEW janitor_detail AS
SELECT j.staff_id, CONCAT(s.first_name, ' ', s.last_name) AS name, j.shift
FROM janitor j
JOIN staff s ON j.staff_id = s.id;

-- Librarian Detail View
CREATE VIEW librarian_detail AS
SELECT CONCAT(s.first_name, ' ', s.last_name) AS librarian_name, l.staff_id AS librarian_id, t.title_name AS librarian_title
FROM librarian l
...
SELECT 
  GROUP_CONCAT(DISTINCT CONCAT(a.first_name, ' ', a.last_name) ORDER BY a.id SEPARATOR ', ') AS Author_name, 
...
SELECT 
    ...
    b.publication_date, 
    b.copies_owned, 
    GROUP_CONCAT(DISTINCT CONCAT(a.first_name, ' ', a.last_name) ORDER BY ba.book_id SEPARATOR ', ') AS author_names,
    c.category_name AS category,
    CONCAT(s.first_name, ' ', s.last_name) AS acquired_by
FROM book b
...
SELECT CONCAT(m.first_name, ' ', m.last_name) AS member_name, 
       ...
       l.loan_date, 
       l.returned_date,
       CONCAT(st.first_name, ' ', st.last_name) AS librarian_incharge
FROM loan l
...
CREATE VIEW fine_payment_report AS
SELECT CONCAT(m.last_name, ' ', m.first_name) AS member_name, 
       fp.payment_amount, 
       fp.payment_date, 
       CONCAT(s.last_name, ' ', s.first_name) AS accountant_name
...
-- Loans Calendar View
CREATE VIEW loans_calendar AS
SELECT CONCAT(m.first_name, ' ', m.last_name) AS member_name, l.loan_date, l.returned_date
FROM loan l
JOIN member m ON l.member_id = m.id;

-- Reservation Calendar View
CREATE VIEW reservations_calendar AS
SELECT CONCAT(m.First_name, ' ', m.Last_name) AS member_name, r.id AS reservation_id, r.Reservation_date
FROM member m
...
CREATE VIEW categories_chart AS
...
CREATE VIEW loans_chart AS
SELECT DATE_FORMAT(loan_date, '%M %Y') AS month_year, COUNT(*) AS num_loans
FROM loan
GROUP BY DATE_FORMAT(loan_date, '%M %Y')
ORDER BY MIN(loan_date);

-- Fine Revenue Chart View
CREATE VIEW fine_revenue_chart AS
SELECT DATE_FORMAT(payment_date, '%M %Y') AS month_year, SUM(payment_amount) AS total_fines
FROM fine_payment
GROUP BY DATE_FORMAT(payment_date, '%M %Y')
ORDER BY MIN(payment_date);

-- Librarian Performance Chart View
CREATE VIEW librarian_performance AS
SELECT s.id AS staff_id, CONCAT(s.first_name, ' ', s.last_name) AS full_name,
       COUNT(DISTINCT l.member_id) AS total_loans, COUNT(DISTINCT l.book_id) AS total_books
FROM staff s
